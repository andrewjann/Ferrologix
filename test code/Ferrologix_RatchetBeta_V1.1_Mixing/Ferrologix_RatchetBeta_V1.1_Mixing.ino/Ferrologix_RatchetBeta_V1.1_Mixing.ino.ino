...
float target_freq = 5;  // Adjust this value CM: change from 5 to 2
float initial_freq = 1;
float initial_pulse = 3/(400*initial_freq); //400 accounts for 400 steps/rotation and 3 accounts for 3:1 gear ratio
...
float ssinitial_pulse = 3/(800*initial_freq);
...
long on_interval = 1000000;
//const long off_interval = 0;
long rev_interval = 0;
const long reset_interval = 5000000;
int j = 1;
...
void RAMP()  {  // motor ramp function
    ...
    if ((initial_freq+rdt) >= target_freq)  {  // If current frequency greater than or equal to target frequency, just set to target frequency
      ...
      current_pulse = 3/(400*current_freq);
...
void state_switch()  {  // motor ramp for state-switching
    ...
    if ((ssinitial_freq+ssrdt) >= sstarget_freq)  {  // If current frequency greater than or equal to target frequency, just set to target frequency
      ...
      sscurrent_pulse = 3/(800*sscurrent_freq);
...
void reset_var()  {  // reset variables for ratchet
  ...
  previous_time = 0;
  initial_freq = 1;
  initial_pulse = 3/(400*initial_freq);
...
void reset_ssvar()  { // reset variables for state-switching
  ...
  ssprevious_time = 0;
  ssinitial_freq = 1;
  ssinitial_pulse = 3/(800*ssinitial_freq);
...
//  ref_time = time;
    ...
    if (pump_current_time - puprevious_time >= rev_interval && pump_current_time - puprevious_time < on_interval) { // counter will start at 0 and increase until 1 second
      rev_interval = rev_interval + 2000000;
      digitalWrite(pump_left[0], HIGH); 
      digitalWrite(pump_left[1], LOW);
    }
    if (pump_current_time - puprevious_time >= on_interval && pump_current_time - puprevious_time < rev_interval) {
      on_interval = on_interval + 2000000;
      digitalWrite(pump_left[0], LOW); 
      digitalWrite(pump_left[1], HIGH);
    }

    if (pump_current_time - puprevious_time >= 31000000) { // at 1 second the following will occur:
      puprevious_time = pump_current_time + reset_interval; // Previous time will set to current time plus 10 sec. This will make current_time - puprevious time negative 
      on_interval = 1000000;
      rev_interval = 0;
      digitalWrite(pump_left[0], LOW); 
      digitalWrite(pump_left[1], LOW);      
